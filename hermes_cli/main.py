...
def cmd_chronos(args):
    ...
    if subcmd == "analyze":
        chronos_analyze(
            ...
            preset=getattr(args, "preset", "premium") or "premium",
...
"""
    ...
    ch_analyze.add_argument(
        "--preset", default="premium",
        choices=["ultra", "premium", "balanced", "xai", "budget"],
        help=(
            "Model tier preset (default: premium). "
            "ultra: Sonnet sims + Opus analysis — max quality, 24 workers | "
            "premium: Haiku sims + Opus analysis — best ratio, 20 workers | "
            "balanced: GPT-4o sims + Opus analysis — cross-provider, 12 workers | "
            "xai: full Grok-4 stack, 16 workers | "
            "budget: Gemini Flash + Haiku"
        ),
    )
